...
public class GameBoard extends Board {
	...
	public GameBoard(Context context) {
		this(context, null, Size.BIG);
	}

	public GameBoard(Context context, boolean[][] matrix, Size size) {
		super(context);
	...
	public void setShipLaF(int x, int y, LafType laf) {
				...
				for (int j = 0; j < 10; j++)
					if (ships[i][j].isNotShip()) {
						if (!global.SHOOTING_TIPS_ENABLED || isFieldShootable(i, j))
							ships[i][j].setLaF(LafType.SHOOTABLE);
						else
							ships[i][j].setLaF(LafType.NOT_SHOOTABLE);
					}
		} else {
			for (int i = 0; i < 10; i++)
				for (int j = 0; j < 10; j++)
	...
	public void printOriginalMatrix() {
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++)
				System.out.print("  " + (shipsMatrix[i][j] ? "S" : "."));
			System.out.println();
		}
		System.out.println();
	}
